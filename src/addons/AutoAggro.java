...
public class AutoAggro extends Thread {
	...
	Coord findCurrentTask() {
		if (m_util.m_ui.fight != null && m_util.m_ui.fight.batk != null)
			return null; // do nothing

		Gob player = m_util.getPlayerGob();

		Moving moveType = player.getattr(Moving.class);
		if (moveType == null || moveType instanceof Following || moveType instanceof Homing)
			return Coord.z; // force back to standing

		if (moveType instanceof LinMove)
			return ((LinMove) moveType).t; // click move on old path
	...
	boolean isAggroed(Gob g) {
		if (m_util.m_ui.fight == null || m_util.m_ui.fight.lsrel == null)
			return false;
