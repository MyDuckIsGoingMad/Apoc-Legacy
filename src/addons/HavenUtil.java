...
public class HavenUtil {
	...
	public void closeClient() {
		...
		while (count < 20 && !stop) {
			...
			wait(500);
			if (count == 10)
				sendErrorMessage("Loging out character in 5 seconds.");
		}
		if (!stop)
			m_ui.sess.close();
	}
	...
	String getResName(Indir<Resource> indir) {
		if (indir.get() != null) {
			...
			int count = 0;
			while (indir.get() == null && count < 1000) {
				wait(50);
				count++;
			}
	...
	public int getFluid(String str) {
		...
		if (str.contains("Empty") || str.contains("empty"))
			return 0;
	...
	public void flowerMenuSelect(String name) {
		...
		for (int i = 0; i < m_ui.flowerMenu.opts.length; i++) {
			if (m_ui.flowerMenu.opts[i].name.contains(name)) {
				m_ui.flowerMenu.wdgmsg(m_ui.flowerMenu, "cl",
						new Object[] { Integer.valueOf(m_ui.flowerMenu.opts[i].num) });
				break;
			}
		}
	}
	...
	public ArrayList<Item> getItemsFromInv(Inventory inv) {
		...
		for (Widget wdg = inv.child; wdg != null; wdg = wdg.next) {
			if (wdg instanceof Item) {
				Item i = (Item) wdg;
				while (!stop && i.missing.loading) {
					wait(100);
					System.out.println("missing shitz");
				}
				list.add(i);
			}
		}
	...
	public ArrayList<Coord> emptyItemArrayExcludeZoneHorizontalOpen(Inventory inv, ArrayList<Item> dropList, int start,
			int end) {
		ArrayList<Coord> invArray = new ArrayList<Coord>();
		ArrayList<Coord> dropSlots = new ArrayList<Coord>();
	...
	public boolean objectSurf(Gob g, Rectangle fattener) {
		...
		while (!stop) {
			...
			if (!PFrunning && !checkPlayerWalking() && getPlayerCoord().equals(c))
				return true;
		}
	...
	public Coord surfaceSpot(Gob object, Coord objectsLocation, boolean boatTravel, boolean lineCheck,
			Rectangle fattener) {
		...
		ArrayList<Point> sPoints = surfacePoints(gobToNegRect(object), objectsLocation, allRect, boatTravel, lineCheck,
				false, fattener);
	...
	public Coord surfaceSpot(Rectangle rect, Coord objectsLocation, boolean boatTravel, boolean lineCheck,
			boolean pclaim) {
		...
		ArrayList<Point> sPoints = surfacePoints(rect, objectsLocation, allRect, boatTravel, lineCheck, pclaim,
				new Rectangle());
		Coord player = getPlayerCoord();
		Point start = new Point(player.x, player.y);
		Point end = new Point(objectsLocation.x, objectsLocation.y);
	...
	public ArrayList<Point> surfacePoints(Rectangle rect, Coord c, ArrayList<Rectangle> allRect, boolean boatTravel,
			boolean lineCheck, boolean pclaim, Rectangle fattener) {
		...
		for (int i = 0; i <= size.x; i++) {
			for (int j = 0; j <= size.y; j++) {
				if (i == 0 || i == size.x || j == 0 || j == size.y) {
					...
					if (!boatTravel && (id == 0 || id == 255))
						continue;
					else if (boatTravel && id > 1)
						continue;
	...
	public ArrayList<Rectangle> getAllHitboxes(boolean boatTravel, boolean claims, Rectangle selfHitbox,
			Gob[] excludeList) { // exluding self hitbox
		ArrayList<Rectangle> hitbox = new ArrayList<Rectangle>();
		hitbox.addAll(getAllObjectRectangles(excludeList));

		if (selfHitbox != null)
			hitboxCorrect(hitbox, selfHitbox); // corrects hitbox arraylist without returning the object

		hitbox.addAll(getAllPathableTileRectangles(boatTravel, null, null));

		if (claims)
			hitbox.addAll(getAllClaimRectangles(boatTravel, false));
	...
	public void hitboxCorrect(ArrayList<Rectangle> hitbox, Rectangle selfHitbox) {
		for (Rectangle r : hitbox) {
			// rects.add(new Rectangle(r.x + selfHitbox.x, r.y + selfHitbox.y, r.width +
			// selfHitbox.width, r.height + selfHitbox.height));
			r.x += selfHitbox.x;
			r.y += selfHitbox.y;
	...
	public ArrayList<Rectangle> getAllObjectRectangles(Gob[] excludeList) {
		...
		for (Gob g : allGob) {
			...
			if (name.contains("/items/"))
				continue;
			if (name.equals("gfx/terobjs/trees/log"))
				continue;
			if (name.equals("gfx/terobjs/blood"))
				continue;
			if (name.equals("gfx/terobjs/claim"))
				continue;
			if (name.equals("gfx/terobjs/anthill-r"))
				continue;
			if ((name.equals("gfx/terobjs/hearth-play") && g.getattr(KinInfo.class) == null))
				continue;
			if ((name.contains("/gates/") && g.GetBlob(0) == 2))
				continue;
			if (objectLifted(g))
				continue;
	...
	public ArrayList<Rectangle> getAllPathableTileRectangles(boolean boatTravel, Coord[] editList,
			Integer[] includeList) {
		Gob player = getPlayerGob();
		...
		for (int Ny = 1; Ny < 299; Ny++) {
			for (int Nx = 1; Nx < 299; Nx++) {
				...
				if (includeList != null) {
					...
					if (waterPath && test < 2)
						continue;
					else if (!waterPath && test < 255 && test > 0)
						continue;
				}
	...
	public int getTileID(Coord c) { // note all tiles are taken by any subtile divided by 11 example: "Coord
									// tileCoord = subtileCoord.div(11);"
	...
	public ArrayList<Line> getLandWaterTiles(boolean returnWaterTiles) { // false will return the land tiles
		...
		for (int Ny = 1; Ny < 299; Ny++) {
			for (int Nx = 1; Nx < 299; Nx++) {
				...
				if (returnWaterTiles && test > 1)
					continue;
				else if (!returnWaterTiles && (test < 2 || 254 < test))
					continue;
	...
	public ArrayList<Gob> superSortGobList(ArrayList<Gob> list, boolean XnotY, boolean folowAxies,
			boolean innerRowFolowAxies) {
		if (list == null)
			return null;
		ArrayList<Gob> sorted = new ArrayList<Gob>();
		ArrayList<Gob> temp = new ArrayList<Gob>();
		int toggle = 1;
		if (innerRowFolowAxies)
			toggle = -1;
		...
		while (list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				Gob g = list.get(i);
				if (g.getr().x == sL && XnotY)
					temp.add(g);
				if (g.getr().y == sL && !XnotY)
					temp.add(g);
			}
	...
	private int largeSmallXY(ArrayList<Gob> list, boolean XorY, boolean folowAxes) {
		...
		if (XorY)
			identifier = list.get(0).getr().x;

		for (Gob g : list) {
			if (identifier < g.getr().x && XorY && !folowAxes)
				identifier = g.getr().x;
			if (identifier > g.getr().x && XorY && folowAxes)
				identifier = g.getr().x;
			if (identifier < g.getr().y && !XorY && !folowAxes)
				identifier = g.getr().y;
			if (identifier > g.getr().y && !XorY && folowAxes)
				identifier = g.getr().y;
		}
	...
	public Coord spiralScanRectFreeSpace(Coord start, Rectangle rectCheck, int maxDist, int increment, boolean myRect) {
		...
		ArrayList<Rectangle> rects = getAllHitboxes(false, false);
		if (myRect)
			rects.add(0, gobToRect(getPlayerGob()));
	...
	public Coord circkleScanRectFreeSpace(Coord start, Rectangle rectCheck, int maxDist, int increment,
			boolean myRect) {
	...
	public void ejectDropObject() {
		...
		while (!stop && checkPlayerCarry() && breakCount < 4) {
			...
			if (c == null)
				c = spiralScanRectFreeSpaceNew(getPlayerCoord(), r, 500, 1, false);
	...
	public int matTransfer(int ToFrom, String signName, String extraName) {
		...
		while (!stop) {
			wait(100);
			if (box.remain != -1 && box.avail != -1 && box.built != -1)
				break;
		}
		...
		if (!stop && ToFrom < 0) {
			emptied = box.avail;
			if ((trans * -1) > box.avail)
				trans = box.avail * -1;
		}

		if (!stop)
			signTransfer(trans, signName, extraName);
	...
	public int matTransfer(int ToFrom, String signName, String extraName, int ignoreWinID) { // negative sucking mats,
																								// posetive filling
		while ((!windowOpen(signName) || windowID(signName) == ignoreWinID) && !stop)
			wait(100);
	...
	public boolean checkEnderTimer(String name) {
		synchronized (TimerController.getInstance().timers) {
			for (Timer timer : TimerController.getInstance().timers) {
				if (timer.getName().contains(name))
					return true;
			}
		}
	...
	public int useCR(int button, Gob CR) { // 0 == no CR; 1 == jumped; 2 == low TW
		...
		while (crossRoadTravel(0) && !stop) {
			...
			if (slenError().contains("You are too tired of travelling."))
				return 2;
		}
	...
	public int fillArea(int itemCount, int columCount, Coord directionOfDepositFromStartC, int columDir,
			String signType, String signName, String extraName, int rowSize, Coord startHere, String itemName) {
		...
		while (itemCount > 0 && !stop) {
			Coord firstSpot = startHere.add(dir.mul((rowSize) * 11))
					.add(dir.abs().swap().mul(columDir).mul(11 * columCount));//
			Gob sign = findClosestObject("sign", firstSpot, firstSpot.add(dir.mul((rowSize - 1) * 11).inv()),
					startHere); //
			...
			while (itemCount > 0 && rowCount > 0 && !stop) {
				...
				if (dir.x == 0) {
					...
					if (getPlayerCoord().x != c.x || firstWalk)
						walkTo(c);
				} else {
					...
					if (getPlayerCoord().y != c.y || firstWalk)
						walkTo(c);
				}
				if (firstWalk) {
					firstWalk = false;
				...
				while (getObjects("sign", getPlayerCoord().add(dir.mul(5)), getPlayerCoord().add(dir.mul(5)))
						.size() == 0 && !stop)
					wait(100);

				if (!stop)
					sign = findClosestObject("sign", getPlayerCoord().add(dir.mul(5)),
							getPlayerCoord().add(dir.mul(5)));

				rowCount--;
			}

			if (itemCount > 0 && !restart)
				columCount++;
		}

		if (dir.y == 0) {
			Coord c = new Coord(startHere.x, getPlayerCoord().y);
			if (getPlayerCoord().x != c.x)
				goToWorldCoord(c);
		} else {
			Coord c = new Coord(getPlayerCoord().x, startHere.y);
			if (getPlayerCoord().y != c.y)
				goToWorldCoord(c);
		}
	...
	public void reGear(int gear, Gob container) {
		...
		while (redoCount < 10 && redo && !stop) {
			...
			for (Item i : itemList) {
				if (i.GetResName().contains("axe"))
					axe = i;
				if (i.GetResName().contains("shovel"))
					shovel = i;
				if (i.GetResName().contains("merchant"))
					merchant = i;
				if (i.GetResName().contains("sack") && sack1 == null)
					sack1 = i;
				if (i.GetResName().contains("sack") && sack1 != null)
					sack2 = i;
				if (i.GetResName().contains("flask"))
					flask = i;
				if (i.GetResName().contains("waterskin"))
					waterskin = i;
				if (i.GetResName().equals("gfx/invobjs/saw"))
					metalsaw = i;
				if (i.GetResName().contains("bucket"))
					bucket = i;
				if (i.GetResName().contains("cloak-hide"))
					cloakhide = i;
				if (i.GetResName().contains("sword"))
					sword = i;
				if (i.GetResName().contains("scythe"))
					scythe = i;
				if (i.GetResName().contains("fryingpan"))
					fryingpan = i;
			}
			...
			if (gear == 0) { // nothing
				...
				if (sack1 == null || sack2 == null || merchant == null)
					redo = true;
				else {
					autoMouseEquipPlayer(sack1);
					wait(100);
				...
				if (flask == null || bucket == null || shovel == null || cloakhide == null)
					redo = true;
				else {
					autoMouseEquipPlayer(shovel);
				...
				if (sack1 == null || sword == null || merchant == null)
					redo = true;
				else {
					autoMouseEquipPlayer(sack1);
					wait(100);
				...
				if (waterskin == null || bucket == null || scythe == null || merchant == null)
					redo = true;
				else {
				...
				if (sack1 == null || fryingpan == null || merchant == null)
					redo = true;
				else {
					autoMouseEquipPlayer(sack1);
					wait(100);
		...
		for (Item i : list) {
			if (i != null) {
				...
				if (gear == 1) { // axe only
					...
					if (itemName.contains("sack") && sack1)
						sack2 = true;
					if (itemName.contains("sack"))
						sack1 = true;

					if (sack1 && sack2)
						return true;
				} else if (gear == 3) { // trav merch
					if (itemName.contains("merchant"))
						merchant = true;
					if (itemName.contains("sack") && sack1)
						sack2 = true;
					if (itemName.contains("sack"))
					...
					if (merchant && sack1 && sack2)
						return true;
				} else if (gear == 4) { // shovel flask
					if (itemName.contains("shovel"))
						shovel = true;
					if (getItemFromBag("flask") != null)
						flask = true;

					if (shovel && flask)
						return true;
				} else if (gear == 5) { // metal saw
					if (itemName.equals("gfx/invobjs/saw"))
						return true;

				} else if (gear == 6) { // axe shovel
					if (itemName.contains("axe"))
						axe = true;
					if (getItemFromBag("shovel") != null)
						shovel = true;
					...
					if (itemName.contains("shovel"))
						shovel = true;
					if (itemName.contains("cloak-hide"))
						cloakhide = true;
					if (getItemFromBag("flask") != null)
						flask = true;
					if (getItemFromBag("bucket") != null)
						bucket = true;

					if (flask && bucket && cloakhide && shovel)
						return true;
				} else if (gear == 8) { // trav merch sword
					if (itemName.contains("merchant"))
						merchant = true;
					...
					if (merchant && sack1 && sword)
						return true;
				} else if (gear == 9) { // trav merch sword
					if (itemName.contains("scythe"))
						scythe = true;
					if (itemName.contains("merchant"))
						merchant = true;
					if (getItemFromBag("flask") != null)
					...
					if (scythe && merchant && flask && bucket)
						return true;
					...
					if (itemName.contains("sack"))
						sack1 = true;
					if (itemName.contains("fryingpan"))
						fryingpan = true;
					if (itemName.contains("merchant"))
						merchant = true;

					if (sack1 && fryingpan && merchant)
						return true;
				}
			}
		}
	...
	public void matsFromToBoat(boolean loadBoat, Coord dropCoord, Gob boat) {
		...
		if (!stop) {
			...
			if (chest == null || stop)
				return;
			if (!stop)
				clickWorldObject(3, chest);
			while (!windowOpen("Chest") && !stop)
				wait(200);
			...
			for (int i = 0; i < 48 && !stop; i++) {
				if (!loadBoat) {
					...
					if (itemCount(chestInv) == 0)
						break;
					if (getPlayerBagSpace() == 0)
						break;
				} else if (loadBoat) {
					transferItemTo(chestInv, 1);
					if (getInvSpace(chestInv) == 0)
						break;
					if (getPlayerBagItems() == 0)
						break;
				}
			}
	...
	public void matsFromToBoat(boolean loadBoat, Gob boat) {
		...
		if (!stop) {
			...
			if (stop)
				return;
			if (!stop)
				clickWorldObject(3, groundChest);
			while (!windowOpen("Chest") && !stop)
				wait(200);
			...
			// while(!(getCursor().contains("chi")) && !stop) wait(200);
			if (!stop)
				clickWorldObject(1, groundChest);
			while (!checkPlayerCarry() && !stop)
				wait(200);
		}
		if (!stop)
	...
	public void advEater(Inventory inv, ArrayList<Item> foodList) {
		...
		for (Item i : foodList) {
			...
			if ((hunger + filler) < stopAt) {
				...
				// System.out.println("i.GetResName() "+ i.GetResName() + " filler "+filler +"
				// hunger"+ hunger);
			}
	...
	public Inventory walkToContainer(Gob container, String name) {
		...
		while (!stop && redo) {
			...
			while (!stop && invCub == null && count < 40) {
				...
				if (!PFrunning && !checkPlayerWalking())
					count++;
				else
					count = 0;
	...
	public Gob carpetIdol() {
		...
		if (idol != null) {
			...
			if (carpet != null && idol.getr().add(0, 11).equals(carpet.getr()))
				return idol;
		}
	...
	public Inventory advWalkToContainer(ArrayList<Gob> containerList, Gob container) {
		...
		while (!stop && dir == null) {
			...
			for (Gob g : containerList) {
				if (container.id == g.id)
					continue;

				if (g.getr().x == container.getr().x) {
					if (g.getr().dist(container.getr()) < dist)
						xCount++;
		...
		if (!goToSpot.equals(getPlayerCoord())) {
			...
			while (windowOpen(windowName) && !stop)
				wait(200);
	...
	public boolean pickupBlocks(int quantity, Coord start, Coord end) {
		...
		// ArrayList<Gob> sortedList = m_util.superSortGobList(signList, true, false,
		// true);
	...
	public void walkToView(Coord c) {
		if (!insideViewCondition(c)) {
			...
			while (!stop && !insideViewCondition(c))
				wait(100);
	...
	public boolean barterTransfer(boolean extract) {
		while (!windowOpen("Barter Stand") && !stop)
			wait(100);
		wait(500);
	...
	public void drinkFromContainer(Gob container, boolean waterDrinkOnly) {
		...
		if (!windowOpen(containerType))
			walkToContainer(container, containerType);
		if (getStamina() < 90) {
			if (!windowOpen(containerType)) {
				clickWorldObject(3, container);
				while (!windowOpen(containerType) && !stop)
					wait(200);
			}
	...
	public void waterStation(String containerType) {
		...
		if (flask == null)
			flask = getItemFromInventory(inv, "waterskin");
	...
	public void waterTransfer(boolean fillInventoryBuckets) {
		...
		if (containerInv == null)
			containerInv = getInventory("Chest");
