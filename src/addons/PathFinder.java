...
public class PathFinder {
	...
	ArrayList<Point> pathFind(ArrayList<Rectangle> allObsticles, Point start, Point end) {
		...
		while (true) {
			...
			if (!cornerAdded) {
				...
				// m_globalCornerList.addAll( setClosedCorners() );
				//// System.out.println("Pilot compleate. " + (System.currentTimeMillis() -
				// time) );
				// System.out.println("m_globalCornerList size. " + m_globalCornerList.size() );
		...
		while (true && backtrack != null) {
			// System.out.println("backTrack " + backtrack.lenth + " first check " +
			// backtrack.first );
			if (backtrack.first)
				break;
		...
		//// System.out.println("Pathfinder finished. " + (System.currentTimeMillis() -
		// time) );
		return path;
	}
	...
	void openCorners(classPath from, ArrayList<classPath> closedList, Point end) {
		...
		for (classPath to : closedList) {
			...
			if (to.first)
				continue;
			if (to.checkLenth(from) + to.dist(end) >= m_globalPathLenth)
				continue;
			if (to.checkLenth(from) > to.lenth)
				continue;
	...
	void addToOpenRect(classRect rect) {
		...
		for (classRect test : m_globalRect) {
			...
			if (!m_openRectList.contains(test)) {
				if (test.centerPoint.distance(rect.centerPoint) > test.rectDist + rect.rectDist)
					continue;
	...
	public class classPath {
		...
		void update(classPath from) {
			if (parrent == null) {
				...
				// System.out.println("update miniupdate " + checkLenth(from) + " vs original <
				// " + lenth + " tot " + ( lenth - checkLenth(from) ) );
				setLenth(from);
				parrent = from;
				checked = false;
			} /*
				 * else{
				 * System.out.println("fail miniupdate " + checkLenth(from) + " vs original > "
				 * + lenth );
				 * }
				 */
		}
	...
	public class classRect {
		...
		void getCorners() {
			...
			corners.add(
					new Corner(new Point(c.x + original.width, c.y), new Point2D.Double(c2.x + shrunk.width, c2.y)));
			corners.add(
					new Corner(new Point(c.x, c.y + original.height), new Point2D.Double(c2.x, c2.y + shrunk.height)));
			corners.add(new Corner(new Point(c.x + original.width, c.y + original.height),
					new Point2D.Double(c2.x + shrunk.width, c2.y + shrunk.height)));
		}
