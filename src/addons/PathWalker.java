...
public class PathWalker extends Thread {
	...
	public void run() {
		if (m_gob != null) {
			...
			if (m_surfaceGob != null)
				toSurface(m_c, m_surfaceGob);
			else
				to(m_c);
		} else {
	...
	void to(Coord c) {
		...
		while (/* rePath && */m_util.pathing) {
			...
			if (m_util.pathing && m_inside == null && getInside(allRect, start, waterPath))
				continue;
			if (m_util.pathing && m_inside != null)
				start = insideFix(p, path, allRect, waterPath);
			if (m_util.pathing)
				...
				if (path != null && m_util.pathing) {
					...
					if (m_util.pathing && !rePath) {
						if (m_action != null) {
							...
							m_util.m_ui.mainview.wdgmsg("itemact", Coord.z.add(200, 200), g.getr(), m_modflag, g.id,
									g.getr());
							...
							if (!rePath) {
								while (!m_util.flowerMenuReady() && m_util.pathing)
									m_util.wait(100);
								if (m_util.pathing)
	...
	void toSurface(Coord c, Gob g) {
		 ...
		 * if(all.getc().equals(m_util.getPlayerCoord() ) && all.id !=
		 * m_util.getPlayerGob().id ){
		 * g = all;
		 * break;
		 * }
		 * }
		...
		if (g != null/* && m_util.boxFree(c, g) */) {
			...
			while (/* rePath && */m_util.pathing) {
				...
				if (m_util.pathing)
					path = pf.pathFind(allRect, start, end);
				if (m_util.pathing)
					path = addSurface(path, g, c, allRect);
				if (m_util.pathing && m_inside != null)
					path.add(0, start);
	...
	Point insideFix(Coord p, ArrayList<Point> path, ArrayList<Rectangle> allRect, boolean boatTravel) {
		...
		// System.out.println("1 " + plist.size() + " " + pcList.size() + " " +
		// m_inside.resname() + " " + m_util.gobToNegRect(m_inside) );
		if (surf != null) {
	...
	ArrayList<Point> surfaceGob(Rectangle rect, Coord c, ArrayList<Rectangle> allRect, boolean boatTravel,
			boolean lineCheck) {
	...
	ArrayList<Point> surface(Rectangle rect, Coord c, ArrayList<Rectangle> allRect, boolean boatTravel,
			boolean lineCheck) {
		ArrayList<Point> surfacePoints = new ArrayList<Point>();
		double delta = 0.1;
	...
	boolean walkPath(ArrayList<Coord> pathCoord, boolean waterPath, int ignoreLast) {
		Gob boat = null;
		if (waterPath)
			boat = m_util.findClosestObject("boat");
	...
	void getAllNegs(ArrayList<Rectangle> allRect, Gob target, Point start, Point end, boolean waterPath,
			boolean repath) {
		...
		synchronized (m_util.m_ui.mainview.glob.oc) {
			for (Gob g : m_util.m_ui.mainview.glob.oc) {
				...
				if (repath && m_memGobs.contains(g))
					continue;
				...
				if (name.contains("gfx/kritter/") || name.contains("gfx/borka/s")) {
					...
					if (r != null && deltaFix(r, g, start, end))
						m_critterGobs.add(r);
		...
		// int rad = 75;
		// ArrayList<Coord> tiles =
		// m_util.getTilesInRegion(player.getc().sub(rad*11,rad*11),
		// player.getc().add(rad*11,rad*11) , 0);
		...
		if (m_pclaims)
			allRect.addAll(m_util.getAllClaimRectangles(waterPath, false));
	...
	boolean goToWorldCoord(Coord c, Coord next) {
		...
		if (!mc.equals(c)) {
			...
			int miniCount = 0;
			/*
			 * while(m_util.getPlayerCoord().equals(mc) &&
			 * !m_util.getPlayerCoord().equals(c) && m_util.pathing){
			...
			while ((m_util.checkPlayerWalking() || frameError || !m_util.getPlayerCoord().equals(c))
					&& m_util.pathing) {
				...
				//////// unstable test code
				/*
				 * if(next != null) if(!frameError && c.dist(next) > 10 &&
				 * c.dist(m_util.getPlayerCoord() ) <= 10 ) break;
				 * 
	 ...
	 * 
	 * if(next != null) if(next.dist(c) < m_nextDist){ split = next.dist(c)/2;
	 ...
	 * 
	 * if(m_util.waterFlaskInfo(flask) < 0.1 && m_util.pathing )
	 * m_util.fillFlask(flask);
	 * 
	 * if(!m_util.hasHourglass() && m_util.getStamina() < 80 && m_util.pathing &&
	 * m_drinkCount > 20){
	 * m_util.useActionBar();
	 ...
	 * return true;
	 * }
	 * if( name.equals("gfx/terobjs/furniture/cclosed") ||
	 * name.equals("gfx/terobjs/furniture/copen" ) ){
	 * size.x = 7;
	 * size.y = 4;
	 * offcet.x = -3;
	 * offcet.y = -3;
