...
public class SeedbagScript extends Thread {
	...
	void sort(ArrayList<Inventory> list) {
		...
		if (coll != null) {
			...
			while (collection.size() > 1 && !m_util.stop) { // pickup seed and place on top of other seeds to arrange
															// optimal arrangement
		...
		collection.clear();
		for (int invGroup = 0; invGroup < list.size(); invGroup++) { // repeat same procedure to sort seeds inside the
																		// seedbags
			Inventory inv = list.get(invGroup);
			for (Item itm : m_util.getItemsFromInv(inv)) {
				collection.add(new itemSort(itm, invGroup, inv));
		...
		for (int i = 0; i < collection.size(); i++) {
			...
			if (i != collection.size() - 1)
				m_util.dropItemInInv(cl.c, cl.inv);
		}
	}
