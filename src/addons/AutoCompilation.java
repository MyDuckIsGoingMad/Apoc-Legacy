...
public class AutoCompilation extends Thread {
	...
	public void compileRun() {
		...
		if (compiler == null) {
			System.err.println(
					"JDK path not set, please set the installed main directory path of the JDK in options/addons.");
	...
	static void showFiles(File[] files, List<File> sourceFileList) {
		for (File file : files) {
			if (file.isDirectory()) {
				...
				if (file.getName().contains("compiled"))
					continue;
		...
		for (File file : files) {
			if (file.isDirectory()) {
				...
				if (file.getName().endsWith(".class"))
					stringNames.add(folderString + file.getName().replace(".class", ""));
			}
		}
	}
