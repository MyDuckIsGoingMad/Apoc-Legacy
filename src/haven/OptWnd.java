...
public class OptWnd extends Window {
	...
	public OptWnd(Coord c, Widget parent) {
		...
		{ /* CAMERA TAB */
			...
			addinfo("clicktgt", "The Target Seeker", "The camera recenters smoothly where you left-click." + dragcam,
					ctab);
			/* fixedcake arg */
			ctab = cambox.new Tab();
			new Label(new Coord(45, 10), ctab, "Fast");
		...
		{ /* AUDIO TAB */
			...
			final Label sfxvol = new Label(new Coord(35, 69 + (int) (Config.sfxVol * 1.86)), tab,
					String.valueOf(100 - getsfxvol()) + " %");
			final Label musicvol = new Label(new Coord(160, 69 + (int) (Config.musicVol * 1.86)), tab,
					String.valueOf(100 - getsfxvol()) + " %");
			final Label alertVol = new Label(new Coord(285, 69 + (int) (Config.alertVol * 1.86)), tab,
					String.valueOf(100 - getsfxvol()) + " %");
			...
			(new Scrollbar(new Coord(150, 70), 196, tab, 0, 100) {
				{
			...
			(new Scrollbar(new Coord(275, 70), 196, tab, 0, 100) {
				{
					val = 100 - Config.alertVol;
				}
		...
		{ /* HIDE OBJECTS TAB */
			...
			redScroll = new Scrollbar(new Coord(380, 260), 148, tab, 0, 255) {
				{
			...
			redScroll.changed();
			greenScroll = new Scrollbar(new Coord(420, 260), 148, tab, 0, 255) {
				{
			...
			greenScroll.changed();
			blueScroll = new Scrollbar(new Coord(460, 260), 148, tab, 0, 255) {
				{
			...
			blueScroll.changed();
			transScroll = new Scrollbar(new Coord(500, 260), 148, tab, 0, 255) {
				{
		...
		{ /* COMBAT OPTIONS TAB */
			...
			if (Utils.getpref("overviewSort", "overview").equals("closest"))
				overviewSort.check("Closest");
			else if (Utils.getpref("overviewSort", "overview").equals("furthest"))
				overviewSort.check("Furthest");
			else if (Utils.getpref("overviewSort", "overview").equals("alphabetical"))
				overviewSort.check("Alphabetical");
			...
			flask = new TextEntry(new Coord(400, 65), new Coord(50, 20), tab,
					addons.HavenUtil.flaskText(Config.flaskNum)) {
	...
	private void setcamera(String camtype) {
		...
		if (mv != null) {
			...
			if (curcam.equals("orig"))
				mv.cam = mv.new OrigCam();
			else if (curcam.equals("clicktgt"))
				mv.cam = mv.new OrigCam2(args);
			else if (curcam.equals("kingsquest"))
				mv.cam = mv.new WrapCam();
			else if (curcam.equals("border"))
				mv.cam = mv.new BorderCam();
			else if (curcam.equals("predict"))
				mv.cam = mv.new PredictCam();
			else if (curcam.equals("fixed"))
				mv.cam = mv.new FixedCam();
			else if (curcam.equals("cake"))
				mv.cam = mv.new CakeCam();
			else if (curcam.equals("fixedcake"))
				mv.cam = mv.new FixedCakeCam(args);
		}
	}
	...
	private class HideList extends Widget {
		...
		public HideList(Coord c, Coord sz, Widget parent, Set<String> l, String cp) {
			...
			new IButton(new Coord(0, 20), this, charCross[0], charCross[1]) {
				public void click() {
					if (sel != null) {
						Config.remhide(sel);
						Config.saveOptions();
	...
	static void updateCheckBoxes() {
		...
		if (HL != null)
			HL.updateList(Config.hideObjectList);
	}
