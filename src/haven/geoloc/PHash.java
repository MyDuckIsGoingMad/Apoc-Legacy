...
public class PHash {
	...
	public long getHash(BufferedImage img) {
		/*
		 * Reduce size. Like Average Hash, pHash starts with a small image. However, the
		 * image
		 * is larger than 8x8; 32x32 is a good size. This is really done to simplify the
		 * DCT
		...
		/*
		 * Reduce color. The image is reduced to a grayscale just to further simplify
		 * the
		...
		/*
		 * Compute the DCT. The DCT separates the image into a collection of frequencies
		 * and
		...
		/*
		 * Reduce the DCT. While the DCT is 32x32, just keep the top-left 8x8. Those
		 * represent
		 * the lowest frequencies in the picture.
		 * 
		 * Compute the average value. Like the Average Hash, compute the mean DCT value
		 * (using only the 8x8 DCT low-frequency values and excluding the first term
		 * since the
		 * DC coefficient can be significantly different from the other values and will
		 * throw
		 * off the average).
		 */
		double total = 0;
		...
		/*
		 * Further reduce the DCT. This is the magic step. Set the 64 hash bits to 0 or
		 * 1
		 * depending on whether each of the 64 DCT values is above or below the average
		 * value.
		 * The result doesn't tell us the actual low frequencies; it just tells us the
		 * very-rough relative scale of the frequencies to the mean. The result will not
		 * vary
		 * as long as the overall structure of the image remains the same; this can
		 * survive
