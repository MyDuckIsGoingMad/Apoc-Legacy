...
public class MenuGrid extends Widget {
	...
	static String[] pfActList = { "dig", "repair", "carry", "mine", "fish", "harvest", "stoneroad", "grass", "plow",
			"dirt", "atk" };
	...
	public MenuGrid(Coord c, Widget parent) {
		...
		functionbar = new ToolbarWnd(new Coord(50, 300), ui.root, "toolbar2", beltsConfig, 2, KeyEvent.VK_F1, 12,
				new Coord(4, 10));
		numpadbar = new ToolbarWnd(new Coord(100, 300), ui.root, "toolbar3", beltsConfig, 10, KeyEvent.VK_NUMPAD0, 14,
				new Coord(5, 10)) {
				...
				if (ad[0].equals("gps")) {
					...
					for (int i = 0; i < ad.length; i++) { // new
						if (ad[i].contains("atk")) {
							if (!Config.singleAttack && doubleTapAttack(ad))
								return;
							if (Config.singleAttack && singleTapAttack(ad))
								return;
						}
	...
	boolean soakAttack(String[] ad) {
		...
		if (System.currentTimeMillis() - soakTimer < maxSoakTime)
			return soak;
