...
public class UI {
	...
	public void newwidget(int id, String type, Coord c, int parent, Object... args) throws InterruptedException {
		...
		if (type.indexOf('/') >= 0) {
			...
			if (type.contains("ui/barterbox"))
				f = Widget.gettype(type);
			if (type.contains("ui/land2"))
				f = Widget.gettype(type);
			if (type.contains("ui/land"))
				f = Widget.gettype(type);
			if (type.contains("ui/aim"))
				f = Widget.gettype(type);
		} else {
			f = Widget.gettype(type);
		}
	...
	public void wdgmsg(Widget sender, String msg, Object... args) {
		...
		synchronized (this) {
			if (!rwidgets.containsKey(sender)) {
				return;
				// throw(new UIException("Wdgmsg sender (" + sender.getClass().getName() + ") is
				// not in rwidgets", msg, args));
			}
