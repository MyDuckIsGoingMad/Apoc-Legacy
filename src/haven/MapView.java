...
public class MapView extends Widget implements DTarget, Console.Directory {
	...
	class FixedCakeCam extends DragCam {
		...
		public void setpos(MapView mv, Gob player, Coord sz) {
			...
			if (mv.pmousepos != null && (mv.pmousepos.x == 0 || mv.pmousepos.x == sz.x - 1 || mv.pmousepos.y == 0
					|| mv.pmousepos.y == sz.y - 1)) {
				if (size == null || !size.equals(sz)) {
					size = new Coord(sz);
					center = size.div(2);
					diff = center.sub(border);
	...
	public boolean mousedown(Coord c, int button) {
		...
		if (ui.fight != null) {
			if (button == 1) {
				...
				if (ui.mnu != null)
					ui.mnu.soakTimer = System.currentTimeMillis();
				if (ui.mnu != null)
					ui.mnu.moveOn = false;
				if (hit == null && Config.disableMouseAcctions)
					wdgmsg("click", c0, mc, 3, 0); // new
			}
			if (button == 3 && ui.fight.closestTarget(mc))
				return true;
		}
			...
			if (!RootWidget.names_ready) {
				for (KinInfo k : kin) {
					...
					if (gc.isect(Coord.z, sz)) {
						...
						if ((isother && Config.showOtherNames) || (!isother && Config.showNames)
								|| (k.gob == onmouse)) {
							show = Color.WHITE;
		...
		if (Config.showHidden && Config.hide) {
			...
			synchronized (glob.oc) {
				for (Gob gob : glob.oc) {
					...
					if (res == null)
						continue;
					if ((!res.hitbox && !gob.hide) || res.skiphighlight)
						continue;
	...
	void drawCombatInfo(GOut g) {
		if (ui.fight == null || ui.fight.lsrel.size() <= 0)
			return;
			...
			if (rel.gobid != playergob) {
				...
				if (gob != null) {
					if (Config.numericalCombat) {
						...
						if (Config.largeCombatInfo) {
							g.atext(Integer.toString(rel.bal) + "/" + Integer.toString(rel.intns), gc.add(-24, 0), 0,
									1);
	...
	void drawPartyTargeting(GOut g) {
		...
		try {
			...
			for (Map.Entry<Integer, Integer> list : targets.entrySet()) {
				...
				if (!Config.mypartyline && host.id == playergob)
					continue;
				else if (!ui.sess.glob.party.memb.keySet().contains(host.id))
					continue;
	...
	public void drawDynamicRallyLines(GOut g) {
		...
		for (rallyPoints r : ra) {
			count++;
			if (r.type == 1)
				g.chcolor(255, 0, 64, 128);
			if (r.type == 2)
				g.chcolor(0, 255, 64, 128);
			if (r.type == 4)
				g.chcolor(0, 0, 255, 128);
			if (r.type == 3)
				g.chcolor(255, 255, 64, 128);
			if (r.type == 5)
				g.chcolor(255, 0, 255, 128);
			drawradius(g, m2s(r.c).add(oc), 5);

			g.chcolor(255, 0, 64, 200);
			if ((visualCounter / 10) == count)
				g.chcolor(255, 255, 64, 255);
