...
public class Bufflist extends Widget {
	...
	public void sessionPulse() {
		if (!teaLogoffski && !scriptLogoffski)
			return;

		synchronized (ui.sess.glob.buffs) {
			for (Buff b : ui.sess.glob.buffs.values()) {
				Resource res;
				if (b == null || (res = b.res.get()) == null)
					continue;

				if (teaLogoffski && res.name.equals("gfx/hud/buffs/tea") && b.ameter == 100) {
					if (logoffCounter % 250 == 0)
						ui.slen.error("100% Tea buff reached. Logging off.");
					logoffCounter++;
					...
					if (logoffCounter % 250 == 0)
						ui.slen.error("Script finished. Logging off.");
					logoffCounter++;
					if (logoffCounter > 749) {
