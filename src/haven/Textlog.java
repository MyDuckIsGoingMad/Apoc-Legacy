...
public class Textlog extends Widget {
	...
	static RichText.Foundry fnd = new RichText.Foundry(TextAttribute.FAMILY, "Sans Serif", TextAttribute.SIZE, 12,
			...
			for (Text line : lines) {
				...
				if (hasfocus && selection.sel && !endline) {
					...
					for (RichText.Part part : ((RichText) line).parts) {
						...
						if (selRow || endline)
							g.frect(Coord.z.add(margin + xdraw, dsy1), Coord.z.add(wid, ydraw));
	...
	public void append(String line, Color col) {
		...
		rl = fnd.render(line, sz.x - (margin * 2) - sflarp.sz().x, TextAttribute.FOREGROUND, col, TextAttribute.SIZE,
				12);
		synchronized (lines) {
			lines.add(rl);
			...
			synchronized (lines) {
				for (Text line : lines) {
					...
					if (c.isect(cc, new Coord(sz.x - margin, line.sz().y))) {
						if (!selection.sel && (selection.selCharStart != selection.selCharEnd
