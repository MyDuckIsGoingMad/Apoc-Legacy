...
public class MainFrame extends Frame implements Runnable, FSMan {
	...
	public void setTitle(String charname) {
		String str = String.format("Haven and Hearth (%s by Xcom v%s)",
				Config.apocScript ? CLIENT_TITLE_SCRIPT : CLIENT_TITLE_LITE, VERSION);
		if (charname != null) {
			str = charname + " - " + str;
		}
		super.setTitle(str);
	...
	public synchronized static void closeSession(int idx) {
		...
		if (ui != null) {
			if (ui.sess != null)
				ui.sess.close();
			if (ui.login != null)
				ui.login.wdgmsg("eject");
		}

		threads.remove(getThread);
		if (idx == index)
			instance.firstSession();
	}
