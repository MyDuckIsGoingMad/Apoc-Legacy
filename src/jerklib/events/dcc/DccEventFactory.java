	...
	public static DccEvent dcc(MessageEvent event, String ctcpString) {
		...
		if (dccTokenList.size() >= 2) {
			...
			if ("SEND".equals(dccType)
				...
				if (ip != null) {
					return new DccSendEventImpl(filename, ip, port, fileSize, ctcpString, event.getHostName(),
							event.getMessage(), event.getNick(), event.getUserName(), event.getRawEventData(),
							event.getChannel(), event.getSession());
				}
			}
			...
			// DCC RESUME filename port position
				...
				return new DccResumeEventImpl(filename, port, position, ctcpString, event.getHostName(),
						event.getMessage(), event.getNick(), event.getUserName(), event.getRawEventData(),
						event.getChannel(), event.getSession());
			}
			...
			// DCC ACCEPT filename port position
				...
				int port = asInteger(dccTokenList.get(3));
				long position = asLong(dccTokenList.get(4));
				return new DccAcceptEventImpl(filename, port, position, ctcpString, event.getHostName(),
						event.getMessage(), event.getNick(), event.getUserName(), event.getRawEventData(),
						event.getChannel(), event.getSession());
			}
			...
			// DCC CHAT protocol ip port
				...
				if (ip != null) {
					return new DccChatEventImpl(protocol, ip, port, ctcpString, event.getHostName(), event.getMessage(),
							event.getNick(), event.getUserName(), event.getRawEventData(), event.getChannel(),
							event.getSession());
				}
			}
		...
		return new DccUnknownEventImpl(ctcpString, event.getHostName(), event.getMessage(), event.getNick(),
				event.getUserName(), event.getRawEventData(), event.getChannel(), event.getSession());
	}
