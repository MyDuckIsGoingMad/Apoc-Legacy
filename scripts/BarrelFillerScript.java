...
public class BarrelFillerScript extends Thread {
	...
	void bucketHandler(Gob object, boolean barrelFill) {
		...
		int id = -1;
		for (Item container : items) {
			if (!container.GetResName().contains(itemTypeName) && !m_util.stop)
				continue;
			if (cyckleCount <= cyckles)
				break;
			cyckles++;
			...
			if (!m_util.stop)
				m_util.itemActionWorldObject(object, 0);
				...
				while (!m_util.checkPlayerWalking() && !m_util.stop) {
					...
					if (testBreak % 50 == 0) {
						if (m_util.getMouseItem() != null
								&& !m_util.getMouseItem().GetResName().contains(itemTypeName)) {
