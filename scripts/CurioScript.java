...
public class CurioScript extends Thread {
			...
			for (Gob g : allGobs) {
				if (g.resname().contains("gfx/terobjs/herbs/")) {
					...
					if (g.isHuman() && g.id != m_util.getPlayerGob().id && (kin == null || kin.group == 2)) {
						if (checkID(g)) {
							System.out.println("--PLAYER-- found. Location: " + m_mainPath.pathName + " Checkpoint: "
					...
					if (checkID(g) && !m_mainPath.boatTravel) {
						System.out.println("Boar found. Location: " + m_mainPath.pathName + " Checkpoint: "
								+ m_mainPath.checkPointNum);
					}
	...
	void startManager() {
		...
		while (!m_util.stop) {
			...
			pathNum++;
			if (m_mainPathList.size() == pathNum)
				pathNum = 0;
		}
	}
	...
	void dropPickupBoat(boolean pickup) {
		...
		if (pickup) {
			...
			if (!m_util.stop)
				m_util.clickWorldObject(1, boat);
	...
	void LCemptying() {
		if (!m_mainPath.boatFiller && m_mainPath.boatFills > 0)
			return;
	...
	void typeInventory() {
		...
		for (Item i : containers) {
			if (i.GetResName().contains("bucket") && m_util.getFluid(i.tooltip) < 100)
				bucketFill = true;
		}
		if (bucketFill)
			fillWater();
	...
	void fieldWater(int type) {
		...
		if (container == null || m_util.stop)
			return;

		walkToGob(container);
		while (!m_util.stop && !m_util.windowOpen(contName))
			m_util.wait(100);
	...
	void boatTraveler() {
		if (m_mainPath.boatTravel) {
			...
			while (!m_util.checkPlayerSitting() && !m_util.stop)
				m_util.wait(500);
		}
	}
	...
	void herbFiltering() {
		for (Gob g : m_mainPath.herbList) {
			if (herbFilter(g))
				m_mainPath.filterdHerbList.add(g);
		}
	...
	void unloadIntoLC() {
		if (!m_mainPath.boatFiller || m_mainPath.working || (!m_mainPath.homeUnload && !m_mainPath.unloading))
			return;
	...
	void moveBreaker() {
		if ((m_mainPath.lastCheckPointReached && m_mainPath.checkPointReached())
				|| (m_mainPath.homeUnload && !m_mainPath.working)) {
	...
	void dumpCurios() {
		...
		for (int i = 0; i < 2; i++) {
			if (m_util.getItemFromBag("mussel") == null && i == 0)
				continue;
			int rowMod = 1;
			...
			while (redo && breakCount < 2 && !m_util.stop) {
				...
				for (int j = k; j < sortedCubs.size(); j++) {
					...
					if ((m_util.getItemFromBag("mussel") == null && i == 0) || bagCheck() == 0)
						break;
	...
	boolean herbFilter(Gob herb) {
		...
		for (Rectangle r : m_mainPath.ignoreZone) {
			Coord c = herb.getr();
			Rectangle zone = new Rectangle(r.x + m_mainPath.centerCalcCR.x, r.y + m_mainPath.centerCalcCR.y, r.width,
					r.height);
		...
		if (m_mainPath.curioType == 1) {
			if (name.contains("gfx/terobjs/herbs/mussel"))
				return true;
			if (name.contains("gfx/terobjs/herbs/flotsam"))
				return true;

		} else if (m_mainPath.curioType == 2) {
			if (name.contains("gfx/terobjs/herbs/edelweiss"))
				return true;
			if (name.contains("gfx/terobjs/herbs/frogscrown"))
				return true;
			if (name.contains("gfx/terobjs/herbs/glimmermoss"))
				return true;

		} else if (m_mainPath.curioType == 3) {
			if (name.contains("gfx/terobjs/herbs/chimingbluebell"))
				return true;
			if (name.contains("gfx/terobjs/herbs/royaltoadstool"))
				return true;
			if (name.contains("gfx/terobjs/herbs/bloatedbolete"))
				return true;

			// if(name.contains("gfx/terobjs/herbs/ladysmantle") ) return true;

		} else if (m_mainPath.curioType == 4) {
			if (name.contains("gfx/terobjs/herbs/cavebulb"))
				return true;
			if (name.contains("gfx/terobjs/herbs/caveclay"))
				return true;
			...
			if (name.contains("gfx/terobjs/herbs/flotsam"))
				return true;
			if (name.contains("gfx/terobjs/herbs/grayclay"))
				return true;
	...
	private class pathClass extends Thread {
		...
		void updateCheckpoint() {
			if (checkPointNum == 0) {
				...
				checkPointNum++;
				walking = false;
				if (pathList.size() <= checkPointNum)
