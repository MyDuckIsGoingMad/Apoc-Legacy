...
public class CraftScript extends Thread {
	...
	void goToContainer(Gob con, Coord dir) {
		...
		if (con.getr().mul(dir.swap()).x < m_util.getPlayerCoord().mul(dir.swap()).x
				|| con.getr().mul(dir.swap()).y < m_util.getPlayerCoord().mul(dir.swap()).y)
			neg = neg.inv();
	...
	void managePow() {
		...
		if (powTest == null || powTest.resname().equals("gfx/terobjs/powb")) {
			...
			while (!m_util.windowOpen("Pile of Wood") && !m_util.stop)
				m_util.wait(200);
	...
	void manageCrucible() {
		if (!m_util.windowOpen("Crucible")) {
			goToTool();
			while (!m_util.windowOpen("Crucible") && !m_util.stop)
				m_util.wait(200);
		}
	...
	void goToTool() {
		...
		if (m_util.getTileID(m_util.getPlayerCoord().div(11)) == 22) {
			if (m_util.getPlayerCoord().x < m_origin.x)
				i = -1;

			m_util.walkTo(m_origin.add(7 * i, 0));
		} else {
			if (m_util.getPlayerCoord().y < m_origin.y)
				i = -1;
	...
	boolean toolCheck(int tool) {
		...
		if (!m_gob.resname().contains(toolRes))
			return false;
	...
	void manageCauldron(boolean barrelFill) {
		...
		if (m_util.getVmeterAmount(255, false) < 10) {
			...
			while (!m_util.windowOpen("Palisade Cornerpost") && !m_util.windowOpen("Cellar Door") && !m_util.stop)
				m_util.wait(200);
	...
	void barrelCauldron() {
		...
		while (!m_util.stop) {
			barrel = m_util.findClosestObject("gfx/terobjs/barrel", m_origin.add(22, 11), m_origin.add(11 * 11, 22),
					m_origin.sub(0, 1000));
			if (barrel == null) {
				barrel = m_util.findClosestObject("gfx/terobjs/barrel", m_origin.add(-22, 11),
						m_origin.add(-11 * 11, 22), m_origin.sub(0, 1000));
	...
	boolean removeBarrel(Gob barrel) {
		...
		while (!m_util.windowOpen("Barrel") && !m_util.stop) {
			m_util.wait(100);
		}
		String str = new String(m_util.barrelInfo());

		if (m_util.getFluid(str) == 0)
			return true;
		...
		while (!m_util.stop) {
			...
			while (m_util.hasHourglass() && !m_util.stop) {
				...
				if (m_util.getStamina() < 33) {
					...
					if (tool == 0)
						m_util.goToWorldCoord(m_origin.add(0, 11));
					fillStamina();
					if (tool == 0)
						m_util.goToWorldCoord(m_origin.add(0, 11));
					// redo = true;
	...
	void dropHoldingItemInBag() {
		if (m_util.mouseHoldingAnItem()) {
			...
			if (bagSize == 24)
				c = new Coord(6 * 30 - 10, 4 * 30 - 10);
			if (bagSize == 30)
				c = new Coord(6 * 30 - 10, 5 * 30 - 10);
			if (bagSize == 35)
				c = new Coord(7 * 30 - 10, 5 * 30 - 10);
			if (bagSize == 42)
				c = new Coord(7 * 30 - 10, 6 * 30 - 10);
			if (bagSize == 48)
				c = new Coord(8 * 30 - 10, 6 * 30 - 10);
			if (bagSize == 56)
				c = new Coord(8 * 30 - 10, 7 * 30 - 10);
	...
	int dumpCraftebles(int dump, cubData cubInfo, craftData cData) {
		...
		if (cData.secondProd.itemName != null)
			check += m_util.countItemsInBag(cData.secondProd.itemName);
	...
	boolean pickupMats(ArrayList<MatClass> matInfo, cubData cubInfo) {
		...
		if (matInfo.size() > 0)
			maxPrio = matInfo.get(0).maxPriority + 1;
		...
		for (int prio = 0; prio < maxPrio; prio++) {
			...
			if (!m_util.stop)
				matSerch = matTransfer(bag, matInfo, cub, prio, false);
			...
			while (matSerch && breakCyckle < 2 && !m_util.stop) {
				...
				if (viableCub) {
					...
					if (!m_util.stop)
						matSerch = matTransfer(inv, matInfo, cub, prio, true);
				}
				if (!matSerch)
					break;
	...
	ArrayList<MatClass> getMatData(craftData cData) {
		...
		if (matInfo.size() > 0)
			matInfo.get(0).setMaxPrio(cData.getHighestPriority());
	...
	int getCraftQuantity(craftData cData) {
		...
		if (bucketTest) {
			...
			while (!m_util.stop) {
				...
				if (craftSize * (quant + 1) + bucketSpace > bagSpace)
					return quant;
		...
		for (MatClass m : matInfo) {
			...
			if (m.matRes.contains("bucket")) {
				...
				for (Item bucket : invBuckets) { // empty
					if (bucket.GetResName().contains("buckete") && !bucketUsed.contains(bucket)
							&& added < bucketCount) {
						added++;
						list.add(bucket);
						bucketUsed.add(bucket);
	...
	void meatAnimals() {
		...
		while (!m_util.stop) {
			...
			while (!finished && !m_util.stop) {
				...
				if (!m_util.stop)
					dumpNum = dumpCraftebles(dumpNum, dumpMeatInfo, cData);
			}

			if (!m_util.stop)
				dumpNumBones = dumpCraftebles(dumpNumBones, dumpBoneInfo, cData);
	...
	boolean getNextCarcas() {
		...
		if (!m_util.stop)
			m_util.walkTo(cadaver);
		if (!m_util.stop)
			m_util.sendAction("carry");
		if (!m_util.stop)
			m_util.clickWorldObject(1, cadaver);
		while (!m_util.checkPlayerCarry() && !m_util.stop)
			m_util.wait(200);
	...
	Gob getTool() {
		String[] tools = new String[] { "gfx/terobjs/anvil", "gfx/terobjs/mgrind", "gfx/terobjs/cauldron",
				"gfx/terobjs/pow", "gfx/terobjs/alloyer", "gfx/terobjs/churn" };
		if (m_option == 4 || m_option == 5)
			tools = new String[] { "/cdv" };
