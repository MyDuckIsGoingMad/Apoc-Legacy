...
public class FarmScriptV2 extends Thread {
	...
	void harvestCrops() {
		...
		for (int i = 0; i < m_farmList.size() && !m_util.stop; i++) {
			...
			if (((inventoryItems + harvestQuantity) > bagSize || newCrops) && !m_dumpLC && !m_replant
					&& !m_sortPumpkins) {
				if (m_dumpInv) {
					if (m_util.getPlayerBagItems() > 0) {
						if (c.equals(new Coord(0, 0)))
							c = m_util.getPlayerCoord();
						grapeCheck(true);
						teaCheck(c);
						tobaccoCheck(c);
			...
			if (m_util.getStamina() < 60 && !m_replant)
				drinkWater();
	...
	void triggerCleanup(Coord c, boolean pickupAll, Coord nextSpot) {
		...
		if (!r.contains(nextSpot.x, nextSpot.y))
			pickupAll = true;
		...
		for (Gob g : obj) {
			String s = g.resname();
			if (s.contains("gfx/terobjs/items/flaxfibre") || s.contains("gfx/terobjs/items/straw")
					|| s.contains("gfx/terobjs/items/flower-poppy")) {
		...
		if (pickupItemCount >= m_util.getPlayerBagSpace())
			clean = true;
	...
	void unloadHighest(Coord c, String itemName) {
		...
		for (Item transfer : bagItems) {
			if (!dropLowest.contains(transfer))
	...
	private void teaCheck(Coord c) {
		...
		// while(m_util.getItemsFromInv(bag).size() != 0 && !m_util.stop)
		// m_util.wait(200);
	}
	...
	void makeWine() {
		...
		if (press.getr().y < m_util.getPlayerCoord().y)
			invert = invert * -1;
	...
	void emptyPress(Gob press, int invert) {
		if (m_util.getVmeterAmount(0, false) > 75) {
			...
			Gob schest = m_util.findClosestObject("gfx/terobjs/furniture/cclosed", m_pos1.add(0, -100),
					m_pos2.add(0, 100), press.getr());
			if (schest == null)
				return;
			 ...
			 * ArrayList<Item>(m_util.getItemsFromInv(invChest2));
			 * 
			 * for(Item i : items)
	...
	private void unloadInventory(Coord c, int invert, boolean invCheck) {
		...
		if (m_util.getInventory("Chest") != null) {
			...
			while (m_util.getInventory("Chest") != null && !m_util.stop)
				m_util.wait(200);
		}
	...
	Inventory weerdBeet(Gob lc, Inventory chest) {
		...
		if (m_util.mouseHoldingAnItem()) {
			...
			if (dropItemC != null)
				m_util.dropItemInInv(dropItemC, chest);
		} else {
			Item weird = m_util.getItemFromBag("beetrootweird");
	...
	private void plantSeed(Coord plantTile, boolean stamina) {
		...
		if (!m_util.mouseHoldingAnItem() && !m_util.stop) {
			...
			while (seed == null && !m_util.stop && count < 20) {
				...
				if (seed == null)
					seed = m_util.getItemFromBag("carrot");
				if (seed == null)
					seed = m_util.getItemFromBag("onion");
				if (seed == null)
					seed = m_util.getItemFromBag("beetroot");
				if (seed == null)
					seed = m_util.getItemFromBag("peapod");
				m_util.wait(100);
		...
		while (redoPlant && !m_util.stop) {
			...
			while (m_util.getObjects("plants", plantTile, plantTile).size() == 0 && m_util.mouseHoldingAnItem()
					&& !m_util.stop) {
				m_util.wait(50);
				...
				if (m_util.getPlayerCoord().equals(errorSpot) && !m_util.checkPlayerWalking()) {
					...
					if (count > 10) {
						if (flop == 1)
							triggerHandPlow(plantTile, stamina);
						redoPlant = true;
						break;
					}
				}
		...
		/*
		 * while(m_util.getObjects("plants", plantTile, plantTile).size() == 0 &&
		 * m_util.mouseHoldingAnItem() && !m_util.stop){
		 * m_util.wait(10);
		 * //m_util.itemAction(plantTile);
		 * count++;
		 * if(count > 4000){
	...
	Inventory goToChest(Gob chest, int invert) {
		if (!chest.getr().add(-10 * invert, 0).equals(m_util.getPlayerCoord()) || !m_util.windowOpen("Chest")) {
			...
			while (m_util.windowOpen("Chest") && !m_util.stop)
				m_util.wait(100);
	...
	void sortReplant() {
		...
		for (ItemSorter sL : globalSortList) {
			...
			while (planting && !m_util.stop) {
				if (m_util.getStamina() < 70)
					drinkWater();
				int plantNum = m_util.getPlayerBagSpace();
				// System.out.println("plantNum "+plantNum);

				if (getSeedingTiles(m_p1, m_p2).size() < m_util.getPlayerBagSpace())
					plantNum = getSeedingTiles(m_p1, m_p2).size();
				...
				for (int chest = 0; chest < chestList.size() && !m_util.stop; chest++) {
					...
					if (seedTransfer > 0 && invSpace > 0) {
						...
						if (invSpace < 0)
							seedTransfer = seedTransfer + invSpace;
	...
	void safeTransfer(int transfer) {
		...
		/*
		 * int count = 0;
		 * while( ( inventoryItems + transfer) != m_util.getPlayerBagItems() &&
		 * !m_util.stop){
		 * m_util.wait(100);
		 * count++;
		 * if(count > 600){
	...
	void drinkWater() {
		...
		Gob schest = m_util.findClosestObject("gfx/terobjs/furniture/cclosed", m_pos1.add(0, -11 * 1000), m_pos2,
				new Coord(m_pos2.x, m_pos1.y));
		if (schest == null)
			return;
		Coord mem = new Coord(m_util.getPlayerCoord());
		 ...
		 * }else{
		 * while(!(mouseItem.GetResName().equals( m_util.getMouseItem().GetResName() ) )
		 * && !m_util.stop ) m_util.wait(200);
		 * }
		 */
	...
	void dumpFood(int num) {
		...
		while (m_util.windowOpen("Trough") && !m_util.stop) {
			m_util.wait(100);
		}
	...
	boolean troughFull(Gob trough) {
		...
		while (m_util.getVmeterAmount(255, true) == -1 && !m_util.stop) {
			m_util.wait(100);
		}
	...
	ArrayList<Coord> multiField(boolean emptyRows) {
		...
		for (int y = m_p2.div(11).y; y >= m_p1.div(11).y; y--) {
			...
			if (!emptyRows) {
				...
				if (memoryPlant != null && y == m_p1.div(11).y)
					innerFieldCoords.add(p1);
			} else {
	...
	public void areaSelectOnPlayer() {
		...
		while (!m_util.stop) {
			...
			if (cyckle < -1000) {
				m_util.stop = true;
				System.out.println("Area to large 1");
			}
		}
		...
		while (!m_util.stop) {
			...
			if (cyckle > 1000) {
				m_util.stop = true;
				System.out.println("Area to large 2");
			}
		}
		...
		while (!m_util.stop) {
			...
			if (cyckle < -1000) {
				m_util.stop = true;
				System.out.println("Area to large 3");
			}
		}
		...
		while (!m_util.stop) {
			...
			if (cyckle > 1000) {
				m_util.stop = true;
				System.out.println("Area to large 4");
			}
		}
	...
	void beehivePlantation() {
		...
		for (int i = 0; i < m_farmList.size() && !m_util.stop; i++) {
			...
			while (m_util.getPlayerBagSpace() > invSpace - 3 && !m_util.stop)
				m_util.wait(100);
		...
		while (redoPlant && !m_util.stop) {
			...
			while (m_util.getObjects("plants", plantTile, plantTile).size() == 0 && m_util.mouseHoldingAnItem()
				...
				if (m_util.getPlayerCoord().equals(errorSpot) && !m_util.checkPlayerWalking()) {
					...
					if (count > 10) {
						if (flop == 1)
							triggerHandPlow(plantTile, true);
						redoPlant = true;
						break;
					}
				}
	...
	public void pickupHighest(ArrayList<Item> ignore) {
		...
		if (!m_util.mouseHoldingAnItem()) {
			...
			for (Item i : plantList) {
				if (!seedTest(i.GetResName()) || ignore.contains(i))
					continue;
	...
	void beeHiveHarvester() {
		...
		while (!m_util.stop) {
			...
			for (Gob bh : list) {
				...
				// }
				// for(Coord c : hives){
				// Gob bh = m_util.findClosestObject("gfx/terobjs/bhive", c.sub(11,11),
				// c.add(11,11));
	...
	int changeStage() {
		...
		if (cropStage.size() != checkStage.size()) {
			...
			for (int i = 0; i < cropStage.size() && i < checkStage.size(); i++) {
				if (checkStage.get(i) != cropStage.get(i))
					stageCount++;
			}
		}
	...
	void beeHiveCoolDown() {
		...
		// while(!(m_util.getCursor().contains("chi")) && !m_util.stop) {
		// m_util.wait(200); }
	...
	private void farmBuds() {
		...
		while (!m_util.stop) {
			...
			if (m_util.getPlayerBagSpace() < 3)
				unloadInventory(m_util.getPlayerCoord(), 1, false);
