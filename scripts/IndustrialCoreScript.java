...
public class IndustrialCoreScript extends Thread {
	...
	/*
	 * public IndustrialCoreScript(HavenUtil util, int directive, boolean toggle,
	 * int num){
	 * m_util = util;
	 * m_toggel = toggle;
	...
	Gob goToSmelter(int num) {
		...
		} else if (smelter.getr().add(rowWidth, 0).x < m_util.getPlayerCoord().x
				|| smelter.getr().sub(rowWidth, 0).x > m_util.getPlayerCoord().x) {
			m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
			m_util.goToWorldCoord(new Coord(smelter.getr().add(xmul, 0).x, m_util.getPlayerCoord().y));
		} else if (smelter.getr().add(smelterSize, 0).x > m_util.getPlayerCoord().x
				&& smelter.getr().sub(smelterSize, 0).x < m_util.getPlayerCoord().x) {
			// m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
			m_util.goToWorldCoord(new Coord(smelter.getr().add(xmul, 0).x, m_util.getPlayerCoord().y));
		}
		...
		while (m_util.windowOpen("Ore Smelter") && !m_util.stop) {
			m_util.wait(50);
		}
		m_util.clickWorldObject(3, smelter);
		// while(m_util.getInventory("Ore Smelter") == null && !m_util.stop){
		// m_util.wait(100);}
		return smelter;
	}
	...
	Gob goToForge(int num) {
		...
		if (m_upperSpot.sub(13 * 11 - 1, 0).x < m_util.getPlayerCoord().x
				|| m_upperSpot.sub(18 * 11 + 1, 0).x > m_util.getPlayerCoord().x) {
			m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
			m_util.goToWorldCoord(new Coord(forge.getr().add(xmul, 0).x, m_util.getPlayerCoord().y));
		} else if (forge.getr().add(rowWidth, 0).x < m_util.getPlayerCoord().x
				|| forge.getr().sub(rowWidth, 0).x > m_util.getPlayerCoord().x) {
			m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
			m_util.goToWorldCoord(new Coord(forge.getr().add(xmul, 0).x, m_util.getPlayerCoord().y));
		} else if (forge.getr().add(forgeSize, 0).x > m_util.getPlayerCoord().x
				&& forge.getr().sub(forgeSize, 0).x < m_util.getPlayerCoord().x) {
			// m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
			m_util.goToWorldCoord(new Coord(forge.getr().add(xmul, 0).x, m_util.getPlayerCoord().y));
		}
		...
		while (m_util.windowOpen("Finery Forge") && !m_util.stop) {
			m_util.wait(50);
		}
	...
	boolean fillEmptySmelter(int type) {
		...
		while (!m_util.windowOpen(windowName) && !m_util.stop) {
			m_util.wait(100);
		}
		if (m_util.stop)
			return true;
	...
	void coalSmelter(Gob smelter) {
		while (m_util.getVmeterAmount(255, false) == -1 && !m_util.stop)
			m_util.wait(50);
		if (m_util.stop)
			return;
		...
		if (m_util.getVmeterAmount(255, false) == 0) {
			...
			if (!m_util.stop)
				m_util.pickUpItem(i);
			if (!m_util.stop)
				m_util.itemActionWorldObject(smelter, 0);

			while (m_util.countItemsInBag("gfx/invobjs/coal") == items || m_util.mouseHoldingAnItem())
				m_util.wait(200);
		}
	}
	...
	boolean emptyFinishedSmelter(int type) {
		...
		while (invCheck && count < 100 && !m_util.stop) {
			...
			if (itemList == null || itemList.size() < typeSize) {
				invCheck = true;
				// System.out.println("IC detected a smelter with less then "+typeSize+" items.
	...
	void getCoal(int type) {
		...
		while (coal < rSize && !m_util.stop) {
			if (type == 1)
				goToCub(m_coalCub, type);
			if (type == 4)
				goToCub(m_coalCubForge, type);
			...
			if (type == 1) {
				...
				if (m_coalCub >= m_cubList.size())
					m_coalCub = 0;
			} else if (type == 4) {
				m_coalCubForge++;

				if (m_coalCubForge >= m_cubList.size())
					m_coalCubForge = 0;
			}
		}
	}
		...
		if (items > 0) {
			...
			while (!m_util.stop) {
				if (type == 1)
					goToCub(m_coalCub, 1);
				if (type == 2)
					goToCub(m_bloomCub, 2);
				if (type == 3)
					goToCub(m_nuggetCub, 3);
				if (type == 4)
					goToCub(m_coalCub, 4);
				if (type == 5)
					goToCub(m_barsCub, 5);
				...
				if (type == 1) {
					...
					if (m_coalCub < 0)
						m_coalCub = m_cubList.size() - 1;
				} else if (type == 2) {
					m_bloomCub++;

					if (m_bloomCub >= m_cubListBloom.size())
						m_bloomCub = 0;
				} else if (type == 3) {
					m_nuggetCub++;

					if (m_nuggetCub >= m_cubListNugget.size())
						m_nuggetCub = 0;
				} else if (type == 4) {
					m_coalCubForge--;

					if (m_coalCubForge < 0)
						m_coalCubForge = m_cubList.size() - 1;
				} else if (type == 5) {
					m_barsCub++;
	...
	void goToCub(int cubNumber, int type) {
		...
		Gob cub = null;
		if (type == 1)
			cub = m_cubList.get(cubNumber);
		if (type == 2)
			cub = m_cubListBloom.get(cubNumber);
		if (type == 3)
			cub = m_cubListNugget.get(cubNumber);
		if (type == 4)
			cub = m_cubList.get(m_cubList.size() - 1 - cubNumber);
		if (type == 5)
			cub = m_cubListBars.get(cubNumber);
		...
		while (m_util.windowOpen("Cupboard") && !m_util.stop)
			m_util.wait(100);
	...
	void manageRestCast(boolean dumpRests) {
		...
		if (m_util.getPlayerCoord().x < front.x)
			front = front.add(-14, 0);
	...
	boolean antiSign() {
		...
		if (m_util.getPlayerBagSpace() <= 0 || m_util.stop)
			return true;
		...
		ArrayList<Gob> list = m_util.getObjects("sign", m_upperSpotForge.add(-4 * 11, 0),
				m_upperSpotForge.add(-42 * 11, 26 * 11));
	...
	int castTransfer(int ToFrom, String signName) {
		...
		while (box == null && !m_util.stop) {
			...
			if (box != null)
				if (box.remain + box.avail + box.built <= 0)
					box = null;
		}
		int emptied = 0;
		if (!m_util.stop)
			emptied = box.remain;
		if (!m_util.stop)
			m_util.signTransfer(ToFrom, signName, extraName);
		return emptied;
	}
	...
	void signMetalsV2() {
		...
		m_colum = m_util.fillArea(itemCount, m_colum, dir, columDir, signType, signName, extraName, rowSize, startC,
				itemName);
	...
	void signMetals() {
		...
		m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
		if (m_upperSpotForge.add(-4 * 11, 0).x < m_util.getPlayerCoord().x)
			m_util.goToWorldCoord(m_upperSpotForge.add(-4 * 11, 0));

		int itemCount = m_util.getPlayerBagItems();

		while (!m_util.stop) {
			Gob sign = m_util.findClosestObject("sign", new Coord(startC.x, m_upperSpot.y), startC,
					new Coord(startC.x, m_upperSpot.y));
			int nextRowSize = m_util
					.getObjects("sign", new Coord(startC.add(-11, 0).x, m_upperSpot.y), startC.add(-11, 0)).size();

			// int rowCount = rowSize - m_util.getObjects("sign", new Coord(startC.x,
			// m_upperSpot.y), startC).size();
			int rowCount;
		...
		while (itemCount > 0 && rowCount > 0 && !m_util.stop) {
			...
			if (sign == null) {
				...
				if (first == true) {
					...
					m_util.sendAction("bp", signType);
					m_util.wait(100);
					// m_util.m_ui.mainview.wdgmsg("place", sign.getr().add(dir.mul(11) ),
					// m_util.MOUSE_LEFT_BUTTON, 0);
			...
			while (m_util.getObjects("sign", m_util.getPlayerCoord().sub(dir.mul(5)),
					m_util.getPlayerCoord().sub(dir.mul(5))).size() == 0 && !m_util.stop)
				m_util.wait(100);

			if (!m_util.stop)
				sign = m_util.findClosestObject("sign", m_util.getPlayerCoord().sub(dir.mul(5)),
						m_util.getPlayerCoord().sub(dir.mul(5)));
	...
	boolean getOreFromGround() {
		...
		int oreCount = m_util
				.getObjects("gfx/terobjs/items/ore-iron", m_upperSpot.sub(1000, 0), m_upperSpot.add(100, 0)).size();
		// int player = m_util.getObjects("gfx/borka/s", m_upperSpot.sub( 1000, 0 ),
		// m_upperSpot.add( 100, 0 ) ).size();
		...
		if (oreCount > 0) {
			 ...
			 * m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, m_upperSpot.y));
			 * String s = "";
			 * CleanupScript CS = new CleanupScript(m_util, m_upperSpot.sub( 1000, 22 ),
			 * m_upperSpot.add( 100, 22 ), s, new Coord(0,0) );
			 * CS.run();
			 * m_util.running(true);
			 */
			/* if(!mem.equals(m_util.getPlayerCoord() ) ) */
			if (m_util.countItemsInBag("gfx/invobjs/ore-iron") > 0)
				return true;
		} else {
			noJobb();
		}
	...
	void smartCollect() {
		...
		CleanupScript CSS = new CleanupScript(m_util, m_util.getPlayerCoord().add(-1000, -1000),
				m_util.getPlayerCoord().add(1000, 1000), closest, Coord.z);
		CSS.run();
		m_util.running(true);
	}
	...
	void feedMEEE(Gob vclaim) {
		...
		ArrayList<Gob> list = m_util.getObjects("gfx/terobjs/cupboard", vclaim.getr().add(5 * 11, 0),
				vclaim.getr().add(25 * 11, 0));
	...
	void acctiveManager(int type) {
		while (!m_util.stop) {
			...
			if (type == 1)
				manager = m_rowManager;
			else
				manager = m_rowManagerForge;
			...
			if (nextTask != null) {
				if (type == 1)
					openOperation(nextTask, manager.indexOf(nextTask));
				else
					openOperationForge(nextTask, manager.indexOf(nextTask));
			...
			int oreCount = m_util
					.getObjects("gfx/terobjs/items/ore-iron", m_upperSpot.sub(200, 0), m_upperSpot.add(40, 0)).size();
			s = s + add + oreCount;
	...
	void loadSettings() {
		try {
			...
			if (strLine == null || !strLine.equals("icInfo"))
				return;

			// m_rowManager.clear();
			int num = 0;
			while ((strLine = br.readLine()) != null) {
				if (strLine.equals("endSmelters"))
					break;
				if (num >= m_rowManager.size())
					break;
	...
	int timerPers(sRow sr) {
		...
		if (sr.time == 0 || sr.task == 1)
			return pers;

		if (sr.task == 2 || sr.task == 3)
			timeCheck = 15 * 60000 + 0 * 1000;
		if (sr.task == 4)
			timeCheck = 5 * 60000 + 35 * 1000;

		long remaningTime = sr.time - System.currentTimeMillis();
		// System.out.println("remaningTime "+remaningTime);
		// System.out.println("remaningTime2 "+((double)remaningTime / (double)timeCheck
		// * (double)100));
		if (remaningTime < 0)
			return 0;
