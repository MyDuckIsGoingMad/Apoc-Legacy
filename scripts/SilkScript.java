...
public class SilkScript extends Thread {
	...
	public String[] options = {
			"Silk Start", "Worms To Cubs", "Cook Cocoons", "Sorth Moths", "Fill Cubs With Leafs",
			"Full Silk With backup", "Drop All Cub Mats"
	};
	...
	void goInViaPF() {
		if (m_relativeCoord.equals(new Coord(0, 0))) {
			...
			while (!m_util.stop) {
				...
				if (m_util.getPlayerCoord().equals(gate.getr().add(0, 7)))
					break;
			}
	...
	void getLeavesFromLC(Gob LC) {
		...
		while (!m_util.stop) {
			...
			if (m_util.getPlayerCoord().equals(goTo))
				break;
		}
	...
	Inventory goToCub(int cubNumber) {
		 ...
		 * 
		 * if( !(frontOfCub.y - 20 < m_util.getPlayerCoord().y &&
		 * m_util.getPlayerCoord().y < frontOfCub.y + 20) ){
	...
	void goToBackupCub(int cubNumber, boolean cocoonToggle) {
		...
		if (!(frontOfCub.x - 20 < m_util.getPlayerCoord().x && m_util.getPlayerCoord().x < frontOfCub.x + 2)) {
			if (!cocoonToggle)
				m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, cellarLine));
	...
	void getLeavesFromTree(Gob tree) {
		...
		while (m_util.countItemsInBag("gfx/invobjs/mulberryleaf") < 48 && !m_util.stop)
			m_util.wait(100);
	}
	...
	int transferWormsToCubs(int startAtCub) {
		...
		while (emptyInv > 0 && !m_util.stop) {
			 ...
			 * 
			 * int count = 0;
			 * while(checkCubContent(inv) != 48 && count < 20 && !m_util.stop)
			 * m_util.wait(200);
			 */
	...
	int getSilkWorms(int tableNum) {
		 ...
		 * 
		 * while(transferCompleate < 3 && tableNum < m_herbalistSizeCount &&
		 * !m_util.stop){
	...
	Inventory goToHerbTable(int tableNumber) {
		 ...
		 * }
		 * 
		 * if( !(frontOfTable.y - 11 < m_util.getPlayerCoord().y &&
		 * m_util.getPlayerCoord().y < frontOfTable.y + 11) ){
	...
	int tableEggs() {
		...
		while (tablesFilled < m_herbalistSizeCount && !m_util.stop) {
			...
			if (bagSpace != 0) {
				...
				if (eggTransfersRemaning < m_util.getPlayerBagSize())
					bagSpace = eggTransfersRemaning;
				...
				for (int cub = 0; cub < m_cubSizeCellar /* cubList.size() */ && !m_util.stop; cub++) {
					...
					if (transfer > 0) {
						...
						if (bagSpace < 0)
							System.out.println("error in transfer to many transfered");
					}
				}
				 ...
				 * 
				 * int count = 0;
				 * while( ( invSpace - itemTransfer ) != m_util.getPlayerBagSpace() &&
				 * !m_util.stop){
				 * m_util.wait(100);
				 * count++;
				 * if(count > 500){
			...
			while (eggs > 0 && !m_util.stop) {
				...
				if (first)
					eggTransfersRemaning -= m_util.getItemsFromInv(herbInv).size();
	...
	void rearrangeMoths(Inventory cubInv, ArrayList<Item> rearrangeList) {
		...
		ArrayList<Coord> coords = m_util.emptyItemArrayExcludeZone(cubInv, rearrangeList, new Coord(0, 0),
				new Coord(7, 1));
	...
	int locatePlayer() {
		while (!m_util.stop) {
			if (m_util.findClosestObject("gfx/arch/stairs-cellar") != null)
				return -1;
			if (m_util.findClosestObject("gfx/terobjs/trees/mberry") != null
					&& m_util.findClosestObject("gfx/arch/door-inn") != null)
				return 0;
			if (m_util.findClosestObject("gfx/arch/door-cellar") != null
					&& m_util.findClosestObject("gfx/arch/stairs-inn") != null
					&& m_util.findClosestObject("gfx/arch/door-inn") != null)
				return 1;
	...
	void goToMantionFloor(int floor) {
		...
		if (locatePlayer() != 1) {
			...
			while (locatePlayer() != 1 && !m_util.stop)
				m_util.wait(200);
		} else {
		...
		if (floor == 0) {
			...
			while (locatePlayer() != 0 && !m_util.stop)
				m_util.wait(200);
			// m_util.wait(500);
			// m_util.camReset();
			m_util.loadArea();
		...
		if (floor == 2) {
			...
			while (locatePlayer() != 2 && !m_util.stop)
				m_util.wait(200);
			// m_util.wait(500);
			// m_util.camReset();
			m_util.loadArea();
		...
		while ((m_sortList1.size() > (cyckle * 24) || m_sortList2.size() > (cyckle * 24)) && !m_util.stop) {
			...
			for (int i = 0; i < m_cubSizeCellar && !m_util.stop; i++) {
				...
				if (maleTransfer > 0 || femaleTransfer > 0) {
					...
					if (maleTransfer > 0)
						targetedTransfer(maleTransfer, "Male", inv, true);
					if (femaleTransfer > 0)
						targetedTransfer(femaleTransfer, "Female", inv, true);
			...
			int innerLoop = 0;
			while (((totalMaleTransfer - innerLoop * 6) > 0 && (totalFemaleTransfer - innerLoop * 6) > 0)
					&& !m_util.stop) {
				innerLoop++;
	...
	int targetedTransfer(int transfer, String type, Inventory inv, boolean skipUpperCub) {
		...
		/*
		 * int count = 0;
		 * while( ( invItems - highList.size() ) == m_util.getItemsFromInv(inv).size()
		 * && !m_util.stop){
	...
	int dumpIntoBackupCub(int backupCub) {
		...
		while (redo && !m_util.stop) {
			...
			int trans = cubSpace;
			if (trans > inventoryItems)
				trans = inventoryItems;
	...
	void cookCocoons(int tool) {
		if (tool == 1) {
			m_util.sendAction("craft", "silkfilament");
			manageCauldron();
			while (!m_util.getCraftName().contains("Silk Filament") && !m_util.stop)
				m_util.wait(100);
		...
		if (tool == 2) {
			...
			goToSpinner();
			while (!m_util.getCraftName().contains("Silk Thread") && !m_util.stop)
				m_util.wait(100);
	...
	boolean safeTransferCocoons() {
		...
		if ((inventoryItems + transfer) == cocoonPickupCount)
			return true;
	...
	void manageCauldron() {
		goToCauldron();
		while (!m_util.windowOpen("Cauldron") && !m_util.stop)
			m_util.wait(200);
		...
		if (m_util.getVmeterAmount(255, false) < 15) {
			...
			while (m_util.getObjects("gfx/arch/sign", 30).size() == 0 && !m_util.stop)
				m_util.wait(200);
	...
	void goToCauldron() {
		...
		if (!(cauldron.getr().x - 5 <= m_util.getPlayerCoord().x
				&& cauldron.getr().x + 11 * 3 + 4 >= m_util.getPlayerCoord().x
				&& cauldron.getr().y - 5 <= m_util.getPlayerCoord().y
				&& cauldron.getr().y + 11 * 3 >= m_util.getPlayerCoord().y)) {
			if (!(cellarDoor.getr().x + 11 * 4 - 4 <= m_util.getPlayerCoord().x
					&& cellarDoor.getr().x + 11 * 4 + 4 >= m_util.getPlayerCoord().x))
				m_util.goToWorldCoord(new Coord(m_util.getPlayerCoord().x, cellarDoor.getr().y));
			m_util.goToWorldCoord(new Coord(cellarDoor.getr().x + 11 * 4, m_util.getPlayerCoord().y));
	...
	Gob getDropLC(Gob tree) {
		...
		for (Gob g : list) {
			if (LC == null) {
				if (m_util.getTileID(g.getr().div(11)) == 4)
					LC = g;
	...
	int getTag(Gob LC, Gob tree) {
		 ...
		 * );
		 * ArrayList<Gob> borkaLC = m_util.getObjects("gfx/borka/s",
		 * LC.getr().add(-15,0), LC.getr().add(15,10) );
		 * 
	...
	void sortBarrels() {
		if (bucketTest()) {
			...
			if (!m_util.stop)
				m_util.objectSurf(well);
			// if(!m_util.stop) new MultiScript(m_util, well, false,
			// false).fillInventoryBuckets(well);
			if (!m_util.stop)
				m_util.safeWalkTo(mem);
		}
		goToMantionFloor(1);
		Coord mem2 = m_util.getPlayerCoord();
		Gob barrel = getObject("gfx/terobjs/barrel");
		if (!m_util.stop)
			m_util.objectSurf(barrel);
		// if(!m_util.stop) new SmeltScript(m_util, barrel, true,
		// false).liquidTransfer(true);
		if (!m_util.stop)
			m_util.safeWalkTo(mem2);
	}
	...
	public void run() {
		 ...
		 * if(m_util.getPlayerBagSpace() >= 48){
		 * m_util.
		 * sendSlenMessage("Full cyckle, table eggs/sort backup eggs/ fill leafes.");
		 * m_util.sendSlenMessage("Filling tables with sorted eggs.");
		 * int backup = tableEggs();
		 * //
		 * //m_util.sendSlenMessage("Sorting backup eggs.");
