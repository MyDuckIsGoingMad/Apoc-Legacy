...
public class MineScriptV3 extends Thread {
	...
	/*
	 * public MineScriptV3(HavenUtil util, int dir, Coord Csafe, boolean troll,
	 * boolean refill, Coord origo){
	 * m_util = util;
	 * m_dir = dir;
	...
	void starters() {
		...
		if (m_util.getRallyPoints().rally.size() == 0 || !setSafeSpot())
			m_util.stop = true;

		if (m_origo == null)
			m_util.sendSlenMessage("WARNING! no bonfire set, save/load disabled.");
	}
	...
	void moveOutOfWay() {
		...
		if (c.dist(m_util.getPlayerCoord()) > 50)
	...
	Coord findSide(Coord tile) {
		...
		if (!checkSupportSafety(tile.mul(11)))
			return sideTC;
		...
		for (Coord c : m_open) {
			...
			for (int x = range.sub(size, size).x; x <= range.add(size, size).x; x++) {
				for (int y = range.sub(size, size).y; y <= range.add(size, size).y; y++) {
					if (x >= 0 && x < 300 && y >= 0 && y < 300 && tileList[x][y] != 255)
						freeCount++;
				}
			}
			...
			} else if ((freeCount > free && (free < m_freeTiles || freeCount >= m_tooFree))
					|| (free < m_tooFree && freeCount >= m_freeTiles && dist > rc.dist(m_util.getPlayerCoord()))) {
				best = c;
				free = freeCount;
				dist = rc.dist(m_util.getPlayerCoord());
				// System.out.println(free);
	...
	int mineThisTile(Coord miningSpot) {
		...
		while (redoTile && !m_util.stop) {
			...
			while (m_util.getTileID(miningSpot.div(11)) == 255 && m_util.hasHourglass() && !m_util.stop)
				m_util.wait(200);
			...
			while (m_util.getTileID(miningSpot.div(11)) == 255 && !drinkBreak
					&& (m_util.hasHourglass() || m_util.miniglass) && !m_util.stop) {
				if (m_util.miniglass) {
					count++;
					if (count > 75) {
						drinkBreak = true;
			...
			if (voidTest && testDugOut(miningSpot))
				redoTile = false;
	...
	boolean testDugOut(Coord miningSpot) {
		...
		if (m_util.getPlayerCoord().equals(miningSpot))
			return true;
	...
	void dropHoldingGarbage() {
		...
		if (m_util.getMouseItem().GetResName().contains("bucket")) {
			...
			if (dropItemC != null) {
				...
				while (m_util.mouseHoldingAnItem() && count < 15 && !m_util.stop) {
					m_util.wait(200);
					count++;
				}
			}
		...
		for (Gob support : supports) {
			...
			if (viewTest(c) /* && m_util.checkReachable(c, false, rects) */ ) {
				...
				for (int q = 0; q < 4; q++) {
					...
					if (!viewTest(b)
							/* || !m_util.checkReachable(b, false, rects) */ || !m_util.freePath(a, b, false, rects)) {
	...
	void trollRunning(ArrayList<Coord> list) {
		...
		while (!m_util.stop) {
			...
			while ((!m_util.getPlayerCoord().equals(location) || m_util.checkPlayerWalking()) && !breakLocation
					&& !m_util.stop) {
				m_util.wait(100);
	...
	public void run() {
		...
		System.out.println("Tiles : " + m_tiles + "   Stone Tiles: " + m_stoneTile + "    Dub Count: " + m_dubCount
				+ "    Total Ore Count: " + m_oreCount);
