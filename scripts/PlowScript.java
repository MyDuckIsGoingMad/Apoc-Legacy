...
public class PlowScript extends Thread {
	...
	public void plower(Coord p1, Coord p2) {
		...
		if (findPlow()) {
			...
			while (plowListSize > 0 && !m_util.stop) {
				...
				if (!first || m_plowList.size() < 1)
					plowListSize = m_plowList.size();
			...
			if (!m_util.stop && !m_farmToggle) {
				...
				if (!m_util.stop)
					m_util.clickWorld(3, m_originalPlowSpot);
				m_util.wait(200);
				if (!m_util.stop)
					m_util.clickWorld(3, m_originalPlowSpot);
			}
			...
			if (!m_util.freePath(m_util.getPlayerCoord(), c, false)) {
				...
				while (m_util.PFrunning && !m_util.stop) {
					...
					if (!m_util.getPlayerGob().miniwalk)
						liftCount++;
					if (liftCount > 3 && !m_util.checkPlayerCarry())
						return false;

					if (!m_util.getPlayerGob().miniwalk && !m_util.checkPlayerWalking()
							&& !m_util.getPlayerCoord().equals(c)) {
						redoCount++;
						if (redoCount > 1000)
							break;
					} else {
					...
					if (m_util.getPlayerCoord().equals(error))
						errorCount++;
					if (errorCount > 50)
						return false;
				}
	...
	void repairPlow() {
		...
		if (m_util.countItemsInBag("wood") < 1)
			getBlocks();

		m_util.sendAction("repair");
		while (!(m_util.getCursor().contains("wrench")) && !m_util.stop) {
			m_util.wait(200);
		}
