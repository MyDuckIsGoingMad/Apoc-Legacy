...
public class MiningAssistant extends Thread {
	...
	public String[] options = {
			"Tunnel Assistant", "Mining assistant", "Mining assistant Queue 1", "Mining assistant Queue 2",
			"Mining assistant Queue 3", "Mining Assistant (without HF move)"
	};
	...
	/*
	 * public MiningAssistant(HavenUtil util, Coord p1, Coord p2, int option,
	 * boolean s){
	 * m_util = util;
	 * m_p1 = p1;
	 * m_p2 = p2;
	...
	void MainOp() {
		...
		if (!startAtMine) {
			...
			if (!m_util.stop)
				dropBoatSurface(m_hearth, m_direction);
			if (!m_util.stop && m_util.countItemsInBag("wood") > 0)
				blockDump(true);
		...
		while (!m_util.stop) {
			...
			if (!m_util.stop)
				dumpOreRefreshLoad();
			if (!m_util.stop)
				blocksTransporting = blockCollect(m_pickupCount);
			if (m_util.getTW() > 78 && !m_util.stop)
				fillTWHome();
			if (!m_util.stop)
				pickUpBoat();
	...
	boolean blockCollect(int pickupCount) {
		...
		if (m_dreamBlock && !blocksPickedUp && m_fullAssistant)
			pickupBlocks(1);
		if (m_fullAssistant)
			pickupDreamNugget(pickNuggets * 2, m_dreamBlock);
	...
	boolean walkToSign(Gob sign, ArrayList<Gob> oldSign) {
		...
		while (!m_util.stop) {
			m_util.wait(100);
			if (m_util.windowOpen(name))
				return true;

			if (!m_util.findObject(sign))
				return false;

			if (!m_util.PFrunning && !m_util.checkPlayerWalking())
				redo++;
			else
				redo = 0;
	...
	void blockDump(boolean blocksTransporting) {
		...
		if (!blocksTransporting && !m_dreamBlock)
			return;

		dumpNuggets();
		m_dreamBlock = false;

		if (!blocksTransporting)
			return;
		...
		for (int i = 0; i < 3; i++) {
			if (!m_util.stop && m_util.getPlayerBagItems() > 0)
				dumpInvSign(c);
			if (i < 2)
				m_util.matsFromToBoat(false, m_hearth.getr().add(direction.mul(9)), m_boat);
		}
	}
	...
	void specialTransfer(Inventory chestInv, ArrayList<Item> itemList, int type) {
		...
		if (chestInv == null || itemList == null)
			System.out.println("chestInv: " + chestInv + "  itemList: " + itemList);
	...
	void dumpInvSign(Coord c) {
		...
		while (!m_util.stop && m_util.countItemsInBag("wood") > 0) {
			...
			if (!m_util.stop)
				rema = box.remain;
			if (!m_util.stop && rema > 0)
				m_util.signTransfer(28, signName, extraName);
	...
	void queueDrop(int queueNum) {
		...
		while (noQueue < 5 && !m_util.stop) {
			...
			if (!queueCheck(queueNum, myGob.id, c) && (m_util.checkPlayerWalking() || myGob.getr().equals(queueSpot)))
				noQueue++;
			else
				noQueue = 0;
	...
	boolean queueCheck(int queueNum, int myID, Coord c) {
		...
		for (Gob g : players) {
			if (g.getr().x < (c.x + 33 + 11 * (queueNum - 1)) && g.getr().y < c.y + 11 * 10 + 5 && g.id != myID
					&& helmCheck(g)) {
				return true;
			}
		}
	...
	void refreshPort(boolean boat) {
		...
		if (!m_util.stop && boat)
			dropBoatSurface(hearth, m_direction);
	...
	void dumpOreRefreshLoad() {
		...
		for (int i = 0; i < 3; i++) {
			if (m_util.getPlayerBagItems() > 0) {
				...
				while (!m_util.stop && m_util.findClosestObject("gfx/borka/s", d, d.add(-50, 10)) != null)
					m_util.wait(200);
	...
	public void foodRefresh() {
		...
		for (int cb = m_cubNumFood; cb < cubs.size(); cb++) {
			...
			if (m_refreshments == 0 && m_util.getHunger() > 899)
				return;
		}
	...
	int enoughWater(Inventory cubInv) {
		...
		if (water >= (m_util.countItemsInBag("bucket") * 100))
			return 3;
	...
	void dumpInv() {
		...
		for (Item i : m_util.getItemsFromBag()) {
			if (!i.GetResName().equals("gfx/invobjs/ore-iron"))
				continue;
	...
	void fieldAssistance(Coord m_direction) {
		...
		for (int i = 0; i < 3 && m_pickupCount != 3; i++) {
			...
			while (!m_util.stop) {
				...
				if (m_hearthMoverAcctive)
					moveHF();

				checkTrolls();

				if (m_fullAssistant) {
					if (spot != null && m_pickupCount < 3)
						placeSupport(spot);
				...
				if (!m_util.getPlayerCoord().equals(m_hearth.getr()))
					m_util.walkTo(m_hearth.getr());

				m_util.wait(500);
			}
			m_boat = m_util.findClosestObject("boat");
			// if(i < 2 && !m_util.stop && m_util.getPlayerBagItems() > 0 && m_pickupCount
			// != 3) m_util.matsFromToBoat(true, m_hearth.getr().add(m_direction.mul(9)),
			// m_boat);
			if (i < 2 && !m_util.stop && m_util.getPlayerBagItems() > 0 && m_pickupCount != 3)
				m_util.matsFromToBoat(true, m_boat);
		}
	}
	...
	void moveHF() {
		...
		if (m_fullAssistant)
			moveSign(c, true);
		while (!buildShit(spot, false) && !m_util.stop)
			m_util.wait(100);
		m_util.walkTo(spot);
		m_hearth = getMyHearth();
		if (m_fullAssistant)
	...
	void moveSign(Coord oldCoord, boolean pickup) {
		...
		if (pickup) {
			...
			if (!m_util.stop)
				suckGarbage(d, m_hearth.getr(), 10);
	...
	boolean playerBlocked(Gob object, Gob player) {
		if (m_util.getPlayerGob().id == player.id)
			return false;
	...
	Coord findSupportsSpot() {
		...
		if (!m_fullAssistant) {
			...
			for (Gob g : supportList) {
				...
				if (c.x == m_hearth.getr().x || c.y == m_hearth.getr().y) {
					if (dirGob == null) {
						dirGob = g;
					} else if (m_supportDirection.sum() > 0 && dirGob.getr().mul(m_supportDirection.abs()).sum() < c
							.mul(m_supportDirection.abs()).sum()) {
						dirGob = g;
					} else if (m_supportDirection.sum() < 0 && dirGob.getr().mul(m_supportDirection.abs()).sum() > c
							.mul(m_supportDirection.abs()).sum()) {
						dirGob = g;
		...
		for (Gob support : supportList) {
			if (m_fullAssistant && ignoreSupport(support))
				continue;
		...
		if (hearthMove && !m_disableHearthMove)
			m_hearthMoverAcctive = true;
	...
	void placeSupport(Coord spot) {
		...
		while (!buildShit(spot, true) && !m_util.stop)
			m_util.wait(100);
		// m_util.walkTo(m_hearth.getr() );
	}
	...
	void safeDumpInv() {
		...
		while (!m_util.stop) {
			for (Item i : m_util.getItemsFromBag()) {
				if (!i.GetResName().equals("gfx/invobjs/ore-iron") || ignore.contains(i))
					continue;
			...
			m_util.wait(300);
			if (m_util.countItemsInBag("ore-iron") <= 0)
				break;
		}
	}
	...
	void safeTransferBlocks() {
		...
		while (!m_util.stop) {
			m_util.wait(100);
			if (m_util.countItemsInBag("wood") == 20)
				break;
			count++;
			if (count > 100) {
				count = 0;
	...
	void getMats() {
		...
		// if(!m_util.windowOpen("Palisade Cornerpost") ) m_util.clickWorldObject(3,
		// sign);
		walkToSign(sign);
		safeTransferBlocks();
		// get nuggets
	...
	void ShaftAssistant() {
		...
		while (!m_util.stop) {
			if (!m_util.stop)
				getVidol();
			if (!m_util.stop)
				m_util.walkTo(m_Vclaim.getr().add(-33, 22));
			if (!m_util.stop)
				scanInvForMats(false);
	...
	boolean scanInvForMats(boolean field) {
		...
		if (field && (woodCount < 20 || nuggetCount < 2))
			return false;
		if (field)
			return true;

		if (woodCount < 21)
			pickupBlocks(blockPickup);

		if (!dream || nuggetCount < 2)
			pickupDreamNugget(2 - nuggetCount, !dream);
		if (!wine)
			getWine();
	...
	void pickupDreamNugget(int pickNuggets, boolean pickDream) {
		...
		if (m_cubNumNugget < m_cubNumDream)
			cubNum = m_cubNumNugget;
		else
			cubNum = m_cubNumDream;

		for (int i = cubNum; i < cubList.size(); i++) {
			if ((!pickDream || m_cubNumDream > i) && (nuggets >= pickNuggets || m_cubNumNugget > i))
				continue;
		...
		if (nuggets == pickNuggets && !pickDream)
			return;
	...
	/*
	 * void rectTest(){
	 * Rectangle rectCheck = new Rectangle(m_util.m_safePos.x-44,
	 * m_util.m_safePos.y-44, 88,88);
	 * Coord placeSpot = m_util.spiralScanRectFreeSpace(m_util.m_safePos, rectCheck,
	 * 80, false);
